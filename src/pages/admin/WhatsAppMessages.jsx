...
const WhatsAppMessages = () => {
  ...
  const refreshOpenChat = useCallback(async () => {
    ...
    if (!convo) return;
    try {
      const { data } = await whatsappConversationsAPI.getMessages(convo.id);
      setConvoMsgs(prev => {
  ...
  const loadConvoMessages = async (convo) => {
    ...
    if (convo.bot_id) {
      whatsappAPI.getStatus(convo.bot_id).then(({ data: statusData }) => {
  ...
  const handleDeleteConvo = async (convo, e) => {
    ...
    try {
      await whatsappConversationsAPI.delete(convo.id);
  ...
  const handleConvoReply = async () => {
    ...
    try {
      await whatsappConversationsAPI.reply(selectedConvo.id, convoReply);
      setConvoReply('');
      refreshOpenChat();
      fetchConvos();
      setTimeout(() => {
      ...
      alert('❌ Gagal kirim: ' + (e.response?.data?.message || e.message));
  ...
  const doToggle = async () => {
    ...
    setToggling(true);
    try {
      const { data } = await whatsappConversationsAPI.toggle(selectedConvoRef.current.id);
  ...
  if (convosLoading && convos.length === 0) {
    return (
      <div>
        <div className="data-table-header"><h1>WhatsApp Messages</h1></div>
  ...
  return (
    <motion.div className="messages-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="messages-layout" style={{ marginTop: 16, height: 'calc(100vh - 220px)' }}>
        ...
        <AnimatePresence>
          {selectedConvo ? (
            <motion.div className="message-detail-panel" key="whatsapp-detail"
                ...
                <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
                  ...
                  {selectedConvo.bot_name && (
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
                      ...
                      {botStatuses[selectedConvo.bot_id]?.status === 'connected'
                        ? <><span style={{ color: '#10b981' }}>🟢</span> Online</>
                        : <><span style={{ color: '#ef4444' }}>🔴</span> Offline</>
                      }
                      <HiCog size={12} style={{ cursor: 'pointer', opacity: 0.5 }} onClick={() => window.location.href = '/admin/whatsapp-bots'} title="Edit Bot" />
                    </div>
                  )}
                </div>
              ...
              <div className="message-reply-box">
                <div style={{ display: 'flex', gap: 8 }}>
                  <textarea className="form-control" rows="2" value={convoReply} onChange={e => setConvoReply(e.target.value)}
                    ...
                    disabled={selectedConvo.status === 'ai' || convoSending}
            ...
            <motion.div className="message-detail-panel message-empty-state" key="whatsapp-empty"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            >
              <HiChatAlt2 className="empty-icon" />
              <h3>Select a conversation</h3>
              <p>Choose a WhatsApp conversation to view messages and reply</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
