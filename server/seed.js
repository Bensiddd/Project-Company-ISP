...
await db.run('INSERT IGNORE INTO admin_users (id, username, password_hash, email, full_name, role, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 'superadmin', hash, 'superadmin@maznet.id', 'Super Admin', 'super_admin', 1]);
await db.run('INSERT IGNORE INTO admin_users (id, username, password_hash, email, full_name, role, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)', [2, 'admin', hash, 'admin@maznet.id', 'Admin MAZNET', 'super_admin', 1]);
